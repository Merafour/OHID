...
static uint8_t Code[OHID_PORT_SIZE];
...
int bl_loop(const uint32_t _time_ms)
{
	...
	case OHID_CMD_BASE_SYNC:
		{
			...
			OHIDS_Base_sync(&blPack, OHID_ERR_CODE_NONE, OHID_TARGET_ID, app_size_max>>8, OHID_RUN_MODE_BOOT, (char*)SN, OHID_BL_Version);
			sign_unlock = SIGN_UNLOCK_NONE;
		}
		break;
	...
	case OHID_CMD_IAP_ERASE:
		...
		if(0==hw_flash_erase()) OHIDS_IAP_Erase(&blPack, OHID_ERR_CODE_NONE, 0xFF);
		else OHIDS_IAP_Erase(&blPack, OHID_ERR_CODE_OVER_SIZE, 0xFF); 
		sign_timout_ms = _time_ms+SIGN_TIMOUT_MS;
		mQueue_hid_init(&mq_hid);
		break;
	...
	case OHID_CMD_IAP_RCRC:
		...
		break;
	case OHID_CMD_IAP_JUMP:
		if(SIGN_UNLOCK_BOOT!=sign_unlock) 
		{
			OHIDS_IAP_Jump(&blPack, OHID_ERR_CODE_SIGN_INVALID, 0, 0, 0);
			break;
		}
		address = OHID_get_32b(&blPack, 0);
		Size = OHID_get_32b(&blPack, 4);
		CRC16   = OHID_get_32b(&blPack, 4+4);
		wCRCin  = 0;
		if(((address+Size)>=(appSize)) || (Size<sizeof(VectorTable)) )
		{
			OHIDS_IAP_Jump(&blPack, OHID_ERR_CODE_SIGN_INVALID, address, Size, wCRCin);
			break;
		}
		if((0==address) && (0xFFFFFFFF!=VectorTable.Vector[0]) && (0xFFFFFFFF!=VectorTable.Vector[1]))
		{
		...
		else wCRCin = fw_crc32(ApplicationAddress+address, wCRCin, appSize, Size);
		if(wCRCin==CRC16)
		{
			if((0xFFFFFFFF!=VectorTable.Vector[0]) && (0xFFFFFFFF!=VectorTable.Vector[1]))
			{
				hw_flash_write(ApplicationAddress+0, VectorTable.bin, sizeof(VectorTable.bin));
				VectorTable.Vector[0] = 0xFFFFFFFF;
				VectorTable.Vector[1] = 0xFFFFFFFF;
			}
			OHIDS_IAP_Jump(&blPack, OHID_ERR_CODE_NONE, address, Size, wCRCin);
			bl_send(blPack.bin, OHID_pack_size(blPack));
			hw_delay_ms(3);
			//jump_to_app(ApplicationAddress, app_size_max);
			NVIC_SystemReset();
		}
		OHIDS_IAP_Jump(&blPack, OHID_ERR_CODE_CRC, address, Size, wCRCin);
		break;
	case OHID_CMD_IAP_PROGRAM:
	{
		if(SIGN_UNLOCK_PROGRAM!=sign_unlock)
		{
			OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_SIGN_INVALID, 0, 0, Code);
			break;
		}
		sign_timout_ms = _time_ms+SIGN_TIMOUT_MS;
		address = OHID_get_32b(&blPack, 0);
		Size = OHID_get_16b(&blPack, 4);
		if((address+Size)>=(appSize))
		{
			OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_OVER_SIZE, address, Size, Code);
			break;
		}
		if(OHID_CMD_GET_FLAG(cmd))
		{
			for(len=0; len<Size; len += 4)
			{
				uint8_t buf[4];
				hw_flash_read(ApplicationAddress+address+len, (uint8_t*)buf, 4*sizeof(uint8_t));
				if((0xFF!=buf[0]) || (0xFF!=buf[1]) || (0xFF!=buf[2]) || (0xFF!=buf[3]) ) 
				{
					OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_PROGRAM, address, Size, Code);
					break;
				}
			}
			for(len=0; len<Size; len ++) Code[len] = blPack.ohid.data[4+2+len];
			if(0==address)
			{
				for(len=0; len<sizeof(VectorTable.bin); len ++) VectorTable.bin[len] = Code[len];
				OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_NONE, address, Size, Code);
				bl_send(blPack.bin, OHID_pack_size(blPack));
				hw_flash_write(ApplicationAddress+address+sizeof(VectorTable.bin), &Code[sizeof(VectorTable.bin)], Size-sizeof(VectorTable.bin));
				return 0;
			}
			else
			{
				OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_NONE, address, Size, Code);
				bl_send(blPack.bin, OHID_pack_size(blPack));
				hw_flash_write(ApplicationAddress+address, Code, Size);
				return 0;
			}
		}
		hw_flash_read(ApplicationAddress+address, Code, Size);
		if((0==address) && (0xFFFFFFFF!=VectorTable.Vector[0]) && (0xFFFFFFFF!=VectorTable.Vector[1]))
		{
			for(len=0; len<sizeof(VectorTable.bin); len ++) Code[len] = VectorTable.bin[len];
		}
		OHIDS_IAP_Program(&blPack, OHID_ERR_CODE_NONE, address, Size, Code);
		break;
	}
